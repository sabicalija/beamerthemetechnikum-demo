...
ensure_path('TTFONTS', "$pwd/assets/theme/technikum/assets/fonts/", "$pwd/assets/fonts//");
...
$clean_ext .= " acn acr alg glg glo gls nav ist snm vrb report report.full";

use File::Path qw(remove_tree);

# Ensure inkscape is on PATH for the svg package.
# If a native `inkscape` is not found, check for a Flatpak installation (Linux)
# and create a thin wrapper script in build/.bin/ that delegates to it.
if ($^O ne 'MSWin32' && system("inkscape --version >/dev/null 2>&1") != 0) {
    if (system("flatpak run org.inkscape.Inkscape --version >/dev/null 2>&1") == 0) {
        use File::Path qw(make_path);
        my $wrapper_dir = "$pwd/build/.bin";
        make_path($wrapper_dir) unless -d $wrapper_dir;
        my $wrapper = "$wrapper_dir/inkscape";
        unless (-f $wrapper && -x $wrapper) {
            open(my $fh, '>', $wrapper) or warn "Cannot create inkscape wrapper: $!\n";
            if ($fh) {
...
# Post-build: run texlogsieve for terminal summary, simple report, and full report
$success_cmd = "texlogsieve --only-summary --summary-detail --no-heartbeat --color $out_dir/%A.log; texlogsieve --only-summary --summary-detail --no-heartbeat --no-color $out_dir/%A.log > $out_dir/%A.report; texlogsieve --summary-detail --no-heartbeat --no-color $out_dir/%A.log > $out_dir/%A.report.full";

# Cleanup: minted cache, stray .fls, inkscape wrapper, standalone sub-builds, svg artifacts
add_hook('cleanup', sub {
    remove_tree("$out_dir/_minted") if -d "$out_dir/_minted";
    unlink glob("$out_dir/xelatex*.fls");
    remove_tree("$out_dir/.bin") if -d "$out_dir/.bin";
    remove_tree("$out_dir/svg-inkscape") if -d "$out_dir/svg-inkscape";
    for my $fdb (glob("$out_dir/*/*.fdb_latexmk")) {
        my $dir = dirname($fdb);
        if ($cleanup_mode == 1) {
            remove_tree($dir);
        } else {
            unlink grep { ! /\.pdf$/ } glob("$dir/*");
        }
    }
    return 0;
});
